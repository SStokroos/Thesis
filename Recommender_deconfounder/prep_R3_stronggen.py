...
raw_data = pd.read_csv(os.path.join(DATA_DIR, 'webscope_train.txt'), sep="\t", header=None, 
                       names=['userId', 'songId', 'rating'],engine="python")
test_data = pd.read_csv(os.path.join(DATA_DIR, 'webscope_test.txt'), sep="\t", header=None, 
                       names=['userId', 'songId', 'rating'],engine="python")
...
def numerize(tp):
    uid = list(map(lambda x: user2id.get(x, -1), tp['userId']))  # Use .get to avoid KeyError
    sid = list(map(lambda x: song2id.get(x, -1), tp['songId']))  # Use .get to avoid KeyError
    
    # Create a new DataFrame instead of modifying tp directly
    numerized_data = pd.DataFrame({'uid': uid, 'sid': sid, 'rating': tp['rating'].values})
